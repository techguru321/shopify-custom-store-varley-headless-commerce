...
export default function PeopleBlock({node}) {
  ...
  return (
    <>
      <div
        className="mx-auto  mb-10  flex  max-w-[1240px]  items-center  justify-between  px-5"
        id={node?.ref}
      >
        {node?.sectionHeading.heading && (
          <Heading item={node?.sectionHeading.heading} />
        )}
        {node?.sectionHeading.subheading && (
          <Heading item={node?.sectionHeading.subheading} />
        )}
      </div>
      <section
        data-interval="3"
